...
def process_single_url(url, parameter, folder, conf_context):
  ...
  nst = time()
  try:
    response = requests.get(url, headers=parameter.get('headers'), proxies=parameter.get('proxies'))
  except:
    return False
  ned = time()
  log_out("  * NetPccsing Complete!___T-cst:{" + str((nst - ned)) + "}", conf_context)
  ...
  try:
    ...
    return False
  pdped = time()
  log_out("  * Pdpccsing Complete!___T-cst:{" + str((pdped - pdpst)) + "}", conf_context)
  ...
  for cpaper_li in all_paper_lis:
    ...
    if authors == None:
      error_out("    * paper [" + paper_name + "] : authors can't fetch!", conf_context)
      authors = ["[CAN'T FETCH AUTHOR]"]
    # get doi link
    doi_link = get_doi_link(cpaper_li)
    if doi_link == None:
      error_out("    * paper [" + paper_name + "] : doi-link can't fetch!", conf_context)
  ...
  ppsged = time()
  log_out("  * PppsGetting Complete!___T-cst:{" + str((ppsged - ppsgst)) + "}", conf_context)
  ...
  fttled = time()
  log_out("  * FTTLGetting Complete!___T-cst:{" + str((fttled - fttlst)) + "}", conf_context)

  # write to file
  log_out("  * WTFProcessing>>>>>>", conf_context)
  wtfst = time()
  of = open(
    conf_context['paper_target_position'] + folder + "/" + cconf_ppname + ".md",
    "wt",
    encoding="utf-8"
  )
  ...
  of.close()
  wtfed = time()
  log_out("  * WTFProcessing Complete!___T-cst:{" + str((wtfed - wtfst)) + "}", conf_context)
...
def main_func():
  ...
  for cproc in proc_list:
    ...
    for url in cproc_target_url:
      process_single_url(url, parameter, cproc.get("folder"), conf_context)
